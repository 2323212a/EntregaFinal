using System;
using System.Windows.Forms;

namespace EntregaFinal.RegulaFalsi
{
    public partial class Grado1 : Form
    {
        public Grado1()
        {
            InitializeComponent();
            panel3.Visible = false;
            panel4.Visible = false;
        }

        public Grado1(string funcion) : this()
        {
            lblFuncionRecibida.Text = funcion;
        }

        private void btnEvaluar_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Verifica tu función");

            // Obtener coeficientes
            if (!int.TryParse(txta0.Text, out int a0)) a0 = 0;
            if (!int.TryParse(txta1.Text, out int a1)) a1 = 0;

            if (a0 == 0 && a1 == 0)
            {
                MessageBox.Show("La función no puede ser completamente 0.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (a1 == 0)
            {
                MessageBox.Show("Debe haber al menos un término con x.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Mostrar función formateada
            string resultado = (a1 == 1) ? "x" : (a1 == -1 ? "-x" : $"{a1}x");

            if (a0 != 0)
                resultado += a0 > 0 ? $" + {a0}" : $" - {Math.Abs(a0)}";

            lblResultado.Text = resultado;
            panel3.Visible = true;
            panel4.Visible = true;
        }

        private void btnContinuar1_Click(object sender, EventArgs e)
        {
            if (!double.TryParse(txtL1.Text, out double l1) || !double.TryParse(txtL2.Text, out double l2))
            {
                MessageBox.Show("Ingresa límites válidos.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            double a0 = string.IsNullOrWhiteSpace(txta0.Text) ? 0 : Convert.ToDouble(txta0.Text);
            double a1 = string.IsNullOrWhiteSpace(txta1.Text) ? 0 : Convert.ToDouble(txta1.Text);

            // Solo si tienes una clase llamada Soluciones
            // Asegúrate de que existe o coméntala
            // Soluciones solForm = new Soluciones(lblResultado.Text, lblFuncionRecibida.Text, a0, a1, l1, l2);
            // solForm.Show();
            // this.Hide();

            MessageBox.Show($"Función: {lblResultado.Text}\nLímites: [{l1}, {l2}]");
        }

        private void lblLimpiar1_Click(object sender, EventArgs e)
        {
            txta0.Clear();
            txta1.Clear(); // <-- corregido (antes se borraba txta0 dos veces)
            lblResultado.Text = "Inserta de nuevo";
            panel4.Visible = false;
        }

        private void lblLimpiar2_Click(object sender, EventArgs e)
        {
            txtL1.Clear();
            txtL2.Clear();
        }
    }
}
